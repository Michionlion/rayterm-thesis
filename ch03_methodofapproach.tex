...
A significant limitation, especially in the field of ray-tracing, is that performance can be significantly affected for inherently divergent tasks -- tasks which do not consistently follow the same control flow -- such as traversing an acceleration structure.
...
OptiX allows user-defined single-ray programs to be compiled into a single program, called a ``kernal,'' that runs on the GPU.
...
\littlesection{Components} \label{ch:methods:renderer:parallel:components}
...
\littlesection{Configurability} \label{ch:methods:renderer:parallel:program}
