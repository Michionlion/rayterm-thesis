...
Many images are generated, or rendered, each second; once an image is rendered the \texttt{libtickit} library \cite{libtickitLibrary} is used to display it in a terminal.
...
\subsection{Terminal Output using \texttt{libtickit}}
...
To do this, we use a two-step approach: first, we represent the image using Unicode characters (see Section~\ref{ch:intro:overview:unicode}), then we print those characters to the terminal using a library called \texttt{libtickit} \cite{libtickitLibrary}.
...
The main problems that \texttt{libtickit} solves are ones which need to use terminal escape codes -- short character sequences that tell a terminal emulator to change some specific data or use a different display method.
