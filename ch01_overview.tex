...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.8\textwidth]{impl-images/rtexplore/terminal_output}
...
Additionally, full RGB direct color support is required for accurate color information; depending on Tickit's status, XTerm may be the only compatible terminal emulator.
...
Since monospaced character output (such as in a terminal) is twice as tall as it is wide, the half-block can split a single character into two square pixels that are colored differently: the upper pixel with the background color of the character, and the lower with the character or foreground color of the pixel.
...
Addition and subtraction are calculated on a component-wise basis\,---\,for instance, in three dimensions addition and subtraction can be calculated using Formula~\ref{equation:vector_addition} and Formula~\ref{equation:vector_subtraction}.
...
\name{} attempts to keep most of its implementation simple and straightforward, using classes and greater abstractions only as necessary.
